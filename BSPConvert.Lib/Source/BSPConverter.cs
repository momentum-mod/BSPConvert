...
public sealed class BSPConverter(BSPConverterOptions options, ILogger logger) : IDisposable
{
    ...
    private IEnumerable<int> UpdateSplitFacesRecursive(ILumpObject obj)
    {
        ...
        var node = (Node)obj;
        IEnumerable<int> faces = UpdateSplitFacesRecursive(node.Child1).Concat(UpdateSplitFacesRecursive(node.Child2));
        ...
        foreach (int faceIndex in faces)
        {
            ...
            if (
                IsPlaneCoplanarWithNode(plane, node.Child1)
                && IsPlaneCoplanarWithNode(plane, node.Child2)
    ...
    private bool TryCreateHeadNode(int brushIndex, out int nodeIndex)
    {
        ...
        byte[] data = new byte[Node.GetStructLength(sourceBsp.MapType)];
        var node = new Node(data, sourceBsp.Nodes) { Child1Index = -leafIndex - 1, Child2Index = -leafIndex - 1 };
    ...
    private int GetBrushContents(Texture texture)
    {
        ...
        if (q3Contents.HasFlag(Q3ContentsFlags.CONTENTS_LAVA) || q3Contents.HasFlag(Q3ContentsFlags.CONTENTS_WATER))
        {
    ...
    private void CreatePatchDisplacement(
        int sFaceIndex,
        Vertex[] faceVerts,
        int patchWidth,
        int patchStartVertex,
        Face qFace
    )
    {
    ...
    private int GetMinTesselation(Face qFace)
    {
        ...
        if (
            shaderDict.TryGetValue(texture, out Shader? shader)
            && shader.surfaceFlags.HasFlag(Q3SurfaceFlags.SURF_NONSOLID)
        )
    ...
    {
        byte[] data = new byte[PlaneBSP.GetStructLength(sourceBsp.MapType)];
        var plane = new PlaneBSP(data, sourceBsp.Planes) { Normal = normal, Distance = distance };
    ...
    private int CreatePrimitiveVertices(Vertex[] vertices)
    {
        ...
        foreach (Vertex vertex in vertices)
        {
            ...
            var texInfo = new PrimitiveTextureInfo(bytes, sourceBsp.PrimitiveTextureInfo) { TexCoord = vertex.uv0 };
    ...
    private int CreateTextureInfo(Texture texture, Vector3 uAxis, Vector3 vAxis)
    {
        ...
        if (q3Flags.HasFlag(Q3SurfaceFlags.SURF_SKY))
            textureInfo.Flags |= (int)(
                SourceSurfaceFlags.SURF_SKY | SourceSurfaceFlags.SURF_NOLIGHT | SourceSurfaceFlags.SURF_SKYNOEMIT
            );
    ...
    private void ConvertExternalLightmaps()
    {
        ...
        for (int faceIndex = 0; faceIndex < quakeBsp.Faces.Count; faceIndex++)
        {
            ...
            ShaderStage? stage = shader.stages.FirstOrDefault(x =>
                x.bundles[0].tcGen == TexCoordGen.TCGEN_LIGHTMAP && x.bundles[0].images[0] != "$lightmap"
            );
            if (stage == null)
                continue;
    ...
    private (Vector2, Vector2) GetLightmapExtents(IEnumerable<Vertex> vertices, float lightmapSize)
    {
        ...
        var lmStart = new Vector2(
            (int)Math.Floor(uvMin.X * (lightmapSize - 1)) - LIGHTMAP_PADDING,
            (int)Math.Floor(uvMin.Y * (lightmapSize - 1)) - LIGHTMAP_PADDING
        );
        var lmEnd = new Vector2(
            (int)Math.Ceiling(uvMax.X * (lightmapSize - 1)) + LIGHTMAP_PADDING,
            (int)Math.Ceiling(uvMax.Y * (lightmapSize - 1)) + LIGHTMAP_PADDING
        );
