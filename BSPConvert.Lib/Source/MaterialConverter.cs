...
public class MaterialConverter
{
    ...
    private readonly string[] skySuffixes = ["bk", "dn", "ft", "lf", "rt", "up"];
    ...
    private void AppendShaderParameters(StringBuilder sb, Shader shader)
    {
        ...
        ShaderStage? textureStage = stages.FirstOrDefault(x =>
            x.bundles[0].tcGen is not TexCoordGen.TCGEN_ENVIRONMENT_MAPPED and not TexCoordGen.TCGEN_LIGHTMAP
        );
        ...
        ShaderStage? envMapStage = stages.FirstOrDefault(x =>
            x.bundles[0].tcGen == TexCoordGen.TCGEN_ENVIRONMENT_MAPPED
        );
        if (envMapStage != null)
        {
        ...
        if (firstImageStage != null)
        {
            ...
            if (
                firstImageStage.flags.HasFlag(
                    ShaderStageFlags.GLS_SRCBLEND_SRC_ALPHA | ShaderStageFlags.GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA
                )
            ) // blendFunc blend
        ...
        if (
            ...
            && textureStage
                .bundles[0]
                .texMods.Any(y =>
                    y.type is TexMod.TMOD_SCROLL or TexMod.TMOD_ROTATE or TexMod.TMOD_STRETCH or TexMod.TMOD_SCALE
                )
        )
    ...
    private void ConvertTexMods(StringBuilder sb, ShaderStage texModStage)
    {
        ...
        foreach (TexModInfo texModInfo in texModStage.bundles[0].texMods)
        {
            ...
            if (texModInfo.type is TexMod.TMOD_ROTATE or TexMod.TMOD_SCROLL or TexMod.TMOD_STRETCH or TexMod.TMOD_SCALE)
