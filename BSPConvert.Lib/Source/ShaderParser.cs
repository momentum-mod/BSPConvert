...
public class ShaderParser(string shaderFile)
{
            ...
            switch (split[0].ToLower(System.Globalization.CultureInfo.CurrentCulture))
            {
                ...
                default:
                    if (!split[0].StartsWith("qer", StringComparison.CurrentCultureIgnoreCase))
                        Debug.WriteLine(
                            "Warning: Unknown shader parameter '" + split[0] + "' in shader file: " + shaderFile
                        );
    ...
    private Shader.FogParms ParseFogParms(string[] split)
    {
        ...
        if (
            !float.TryParse(split[2], out fogParms.color.X)
    ...
    {
        var wave = new WaveForm { func = NameToGenFunc(split[0]) };
    ...
    {
        var texModInfo = new TexModInfo { type = TexMod.TMOD_ENTITY_TRANSLATE };
